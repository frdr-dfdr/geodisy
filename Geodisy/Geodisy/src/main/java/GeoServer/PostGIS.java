...
public class PostGIS {
    ...
    public boolean addFile2PostGIS(DataverseJavaObject djo, String fileName, String geoserverLabel) {
        ...
        String call = GeodisyStrings.replaceSlashes(SHP_2_PGSQL + djo.getSimpleFieldVal(RECORD_LABEL) + "/" + fileName + " " + POSTGRES_SCHEMA + geoserverLabel + PSQL_CALL + VECTOR_DB + POSTGIS_USER_CALL);
        ...
        try {
            ...
            if(!error.contains("Unable to convert data value to UTF-8")) {
                ...
                for(String en: encodings){
                    call = GeodisyStrings.replaceSlashes(SHP_2_PGSQL_ALT(en) + djo.getSimpleFieldVal(RECORD_LABEL) + "/" + fileName + " " + POSTGRES_SCHEMA + geoserverLabel + PSQL_CALL + VECTOR_DB + POSTGIS_USER_CALL);
                    pc = new ProcessCall();
                    results = pc.runProcess(call, 10, TimeUnit.SECONDS, logger);
                    error = results[1];
                    if (!error.contains("Unable to convert data value to UTF-8")) {
